...
//锁屏
function lock(){
    getTagName('body').innerHTML += "<div id='black'></div>";
}
//解屏
function unlock(){
    document.body.removeChild(getId('black'));
}
//弹窗
function uploadbox(url, w, h) {
    var t = (screen.height - h) / 2;
    var l = (screen.width - w) / 2;
    window.open(url, "upfile", "width=" + w + ",height=" + h + ",top=" + t + ",left=" + l + ",scrollbars=no,z-look=yes,resizable=no");
}
function exit_info(url) {
    //if(confirm('如果您改变过数据并且没有提交，离开后将不会保存您所做的改变，您确认要离开此页面吗？')){
    location.href = url;
    //}
}
//日历选择
...
function photoUploadDone(data){
    $('input[name=photo]').val(data['realpath']);
    $('#maga-photo').attr('src', '/assets/../upload/magaico/'+data['realpath']);
    $('#maga-photo').show();
}

function uploadCallback(callbackName, data){
    jsonCheck(data);
